...
# 4. 训练 Ridge 回归模型（使用默认参数）
ridge_model = Ridge(alpha=1.0)
ridge_model.fit(X_train_scaled, y_train_scaled.ravel())

# 对训练集和测试集进行预测
train_preds_scaled = ridge_model.predict(X_train_scaled)
test_preds_scaled = ridge_model.predict(X_test_scaled)
...
# 保存结果
results_df.to_csv("Ridge_results_base.csv", index=False)
print("\n预测结果已保存到文件：Ridge_results_base.csv")