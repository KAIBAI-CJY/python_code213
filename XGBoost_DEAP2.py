...
def create_individual():
    ...
    colsample_bytree = np.random.choice([0.1,0.3,0.5,0.7])
...
def validate_params(individual):
    ...
    individual[4] = np.clip(individual[4], 0.0, 1.0)  # colsample_bytree
    return individual
...
def evaluate(individual):
    ...
    n_estimators, max_depth, learning_rate, subsample, colsample_bytree = individual
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(X_train)):
        ...
        # 将数据转换为 DMatrix 格式以支持早停
        dtrain = xgb.DMatrix(X_train_fold_scaled, label=y_train_fold.ravel())
        dvalid = xgb.DMatrix(X_valid_fold_scaled, label=y_valid_fold.ravel())
        ...
        model = xgb.train(
            params=params,
            dtrain=dtrain,
            num_boost_round=100,  # 设置最大迭代次数
            evals=[(dtrain, 'train'), (dvalid, 'valid')],  # 指定训练集和验证集
            early_stopping_rounds=10,  # 如果验证集性能在 30 轮内没有提升，则停止训练
            verbose_eval=False,  # 不打印日志
...
for gen in range(generations):
    ...
    for mutant in offspring:
        if np.random.rand() < mut_prob:
            # 仅对浮点数参数进行变异
            for i in [2, 3, 4]:  # learning_rate, subsample, colsample_bytree
...
# 保存结果
results_df.to_csv("XGBoost_results1.csv", index=False)
print("\n 预测结果和最优超参数已保存到文件：XGBoost_results1.csv")