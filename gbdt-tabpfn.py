...
from tabpfn_extensions.rf_pfn import DecisionTreeTabPFNRegressor  # 确认你的库路径正确
import torch
import seaborn as sns
from metrics import calculate_metrics  # 你自定义的指标计算函数
...
# 初始化基础TabPFN回归模型（叶节点弱学习器）
reg_base = TabPFNRegressor(device=device, random_state=42)

# 初始化并训练DecisionTree-TabPFN回归模型
dt_model = DecisionTreeTabPFNRegressor(tabpfn=reg_base)

dt_model.fit(X_train_scaled, y_train_scaled)

# 预测（标准化后的预测值）
train_preds_scaled = dt_model.predict(X_train_scaled)
test_preds_scaled = dt_model.predict(X_test_scaled)
