...
for i, (file_path, model_name) in enumerate(zip(file_paths, model_names)):
    ...
    # 趋势线函数
    def plot_trendline(x, y, color, label_text):
        coeff = np.polyfit(x, y, 1)
        poly = np.poly1d(coeff)
        x_range = np.linspace(min(x), max(x), 100)
        ax.plot(x_range, poly(x_range), color=color, linestyle='--', linewidth=3, alpha=0.8, label=label_text)
        return coeff

    # 训练集趋势线
    slope_train = np.polyfit(y_train_true, y_train_pred, 1)[0]
    plot_trendline(y_train_true, y_train_pred, color=colors[i],
                   label_text=f'训练集趋势线 (斜率={slope_train:.2f})')

    # 测试集趋势线
    slope_test = np.polyfit(y_test_true, y_test_pred, 1)[0]
    plot_trendline(y_test_true, y_test_pred, color=test_colors[i],
                   label_text=f'测试集趋势线 (斜率={slope_test:.2f})')
    ...
    # 坐标轴设置
    ax.set_xlabel('不可逆阻力实际值（10¹² m⁻¹）', fontsize=LABEL_FONT_SIZE, fontweight='bold')
    ax.set_ylabel('不可逆阻力预测值（10¹² m⁻¹）', fontsize=LABEL_FONT_SIZE, fontweight='bold')
