...
# 模型文件名（未优化 vs. 优化后）
base_files = [
    "01_Ridge_results_base1.xlsx",
    "02_SVM_results_base1.xlsx",
    "03_MLP_results_base1.xlsx",
    "04_GBDT_results_base1.xlsx",
    "05_TabPFN_results_base1.xlsx"  # 新添加的模型
]
deap_files = [
    "01_Ridge_results1.xlsx",
    "02_SVM_results1.xlsx",
    "03_MLP_results1.xlsx",
    "04_GBDT_results1.xlsx"
]
...
for i, model_name in enumerate(model_names):
    for folder, opt_label in zip([base_folder, deap_folder], ["Default", "Tuned(GA)"]):
        ...
        print(f"🔎 训练集大小: {y_train_true.shape}, 预测集大小: {y_train_pred.shape}")
        print(f"🔎 测试集大小: {y_test_true.shape}, 预测集大小: {y_test_pred.shape}")
        # 计算误差
...
for i, metric in enumerate(metrics_names):
    ...
    for dataset, bar_offset in zip(["Train", "Test"], [-0.2, 0.2]):
        ...
        for model in model_names:
            ...
            # 处理可能的空值情况
            base_val = base_val[0] if len(base_val) > 0 else np.nan
            ...
            if model == "TabPFN":
                ...
                tuned_val = tuned_val[0] if len(tuned_val) > 0 else np.nan
    ...
    ax.set_xticklabels(model_names, rotation=0, fontsize=20, fontweight='bold')
...
print(f"✅ 性能对比图已保存至: {output_image_path}")
