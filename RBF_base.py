...
# 5. 训练 RBFN 模型（使用默认超参数）
rbfn_model = SimpleRBFN(n_centers=10, gamma=1.0, alpha=0.1)
rbfn_model.fit(X_train_scaled, y_train_scaled)

# 对训练集和测试集进行预测
train_preds_scaled = rbfn_model.predict(X_train_scaled)
test_preds_scaled = rbfn_model.predict(X_test_scaled)
...
# 保存结果
results_df.to_csv("RBF_results_base4.csv", index=False)
print("\n预测结果已保存到文件：RBF_results_base4.csv")